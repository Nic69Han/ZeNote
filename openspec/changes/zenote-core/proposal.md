...
- **Rappels par déclencheur** plutôt que rappels horaires : « quand je vois Marc », « avant le point du lundi », « à la fin du comité ». Fondé sur les intentions d'implémentation ([Gollwitzer](http://pham315.pbworks.com/f/McDaniel+2008.pdf)). En v1 les déclencheurs viennent de l'agenda et des transitions d'usage ; le déclencheur de lieu ([Place-Its](https://www.researchgate.net/publication/221568777_Place-Its_A_Study_of_Location-Based_Reminders_on_Mobile_Phones)) est reporté, la géolocalisation n'étant pas dans le périmètre v1.
- **Livraison aux points de rupture** : les rappels et suggestions arrivent aux transitions (fin de réunion, fin de créneau d'agenda, reprise de l'appareil après une pause), pas au milieu d'une tâche ([Iqbal & Bailey](https://dl.acm.org/doi/10.1145/1240624.1240732), [Mark et al. 2008](https://ics.uci.edu/~gmark/chi08-mark.pdf)).
...
- pas de géolocalisation ni de rappels de lieu (reportés : ils demandent une permission permanente et un budget d'autonomie que la v1 ne justifie pas) ;
- pas de lecture des e-mails ni des messageries (c'est un produit en soi, pas une fonction) ;
...
- `rappels` : rappels par déclencheur (personne, événement d'agenda, transition) et par échéance, livrés à un moment opportun.
...
- **Surfaces** : application **Android** (capture, revue, maintenant, rappels) et application **Windows** (capture par raccourci global, revue, recherche). Le téléphone est la surface de capture primaire, le PC la surface de traitement.
...
- **Décisions arrêtées** : Android et Windows comme plateformes v1 ; analyse distante par défaut avec refus possible par capture et par sphère ; agenda comme seule source externe (ni position, ni e-mails, ni messageries) ; outil mono-utilisateur, sans compte ni facturation, mais export en format ouvert dès le premier jour.
- **Hypothèses restantes** (consignées, non bloquantes) : français comme langue principale avec anglais en second ; Revue à heure fixe ajustée par l'agenda ; réunions traitées par vidage vocal et import de compte rendu, l'enregistrement de réunion restant sur action explicite.
