...
- [ ] 4.1 Ajouter le réglage « Analyse sur un service distant », **éteint par défaut**, dont l'activation montre avant confirmation ce qui part, vers qui, et ce qui ne part jamais ; réécrire le bloc « Ce qui quitte l'appareil » de `app/src/ui/reglages.ts` pour qu'il dise la vérité dans les deux états. Vérifier par un test d'écran que l'allumage sans confirmation laisse le réglage éteint
